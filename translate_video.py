...
if os.path.isdir(_venv_dir) and sys.executable != _venv_py:
    ...
    print(
        f"Error: .venv python not found or not executable at {_venv_py}",
        file=sys.stderr,
    )
    sys.exit(1)
...
def download_video(url, download_dir):
    ...
    try:
        ...
        print(
            "Error: yt-dlp is required to download videos. Please install yt-dlp.",
            file=sys.stderr,
        )
        sys.exit(1)
...
def extract_audio(video_path, audio_path):
    ...
    sys.stderr.write("extract audio\n")
...
def transcribe_audio(audio_path, model_name):
    ...
    try:
        ...
        print(
            "Error: openai-whisper is required. Please install via 'pip install openai-whisper'.",
            file=sys.stderr,
        )
        sys.exit(1)
    sys.stderr.write("transcribe audio\n")
...
def select_tts_model(target_lang):
    ...
    try:
        ...
        print(
            "Error: Coqui TTS (TTS) is required. Please install via 'pip install TTS'.",
            file=sys.stderr,
        )
        sys.exit(1)

    sys.stderr.write("select TTS\n")
    ...
    if multi_matches:
        print(
            f"Warning: no language-specific TTS model for '{target_lang}', using multilingual model '{multi_matches[0]}'",
            file=sys.stderr,
        )
...
def generate_tts_audio(
    ...
    if "xtts" in model_name.lower():
        if not speaker_wav:
            ...
            print(
                f"Error: a reference WAV (--speaker-wav) is required for XTTS model '{model_name}'",
                file=sys.stderr,
            )
            sys.exit(1)
        tts_kwargs = {
...
def combine_audio_segments(segments, audio_dir, total_duration, output_audio):
    ...
    sys.stderr.write("combine audio segments\n")
...
def merge_video_audio(video_path, audio_path, output_path):
    ...
    sys.stderr.write("Merge audio and video\n")
    ...
    try:
        ...
        print(
            f"Error: failed to merge and speed-adjust video into {output_path}",
            file=sys.stderr,
        )
        sys.exit(1)
...
def main():
    parser = argparse.ArgumentParser(
        description="Translate video speech and replace audio."
    )
    ...
    parser.add_argument(
        "--speaker", help="Speaker name to use for multi-speaker TTS models (non-XTTS)"
    )
    parser.add_argument(
        "--speaker-wav",
        dest="speaker_wav",
        help="Reference WAV file path for voice cloning models (e.g., XTTS)",
    )
    parser.add_argument(
        "--no-edit", action="store_true", help="Skip interactive text correction"
    )
    parser.add_argument(
        "--edits", help="Path to JSON file with per-segment edits (orig and tgt)"
    )
    parser.add_argument(
        "--preview",
        action="store_true",
        help="Play each segment's audio as it's generated and allow early exit",
    )
    parser.add_argument(
        "--segments-file",
        dest="segments_file",
    ...
    parser.add_argument(
        "--wav2lip-checkpoint", help="Path to Wav2Lip checkpoint (.pth) file"
    )
    ...
    if args.lip_sync:
        ...
        if not os.path.isfile(inf):
            print(
                f"Error: inference.py not found in Wav2Lip dir '{wav2lip_dir}'",
                file=sys.stderr,
            )
            sys.exit(1)
        ...
        if not os.path.isfile(ckpt):
            ...
            try:
                ...
                subprocess.run(
                    [sys.executable, "-m", "pip", "install", "gdown"], check=True
                )
                import gdown
            ...
            try:
                gdown.download_folder(
                    id=folder_id, output=ckpt_dir, quiet=False, use_cookies=False
                )
            except Exception as e:
                print(
                    f"Error: failed to download from Google Drive folder {folder_id}: {e}",
                    file=sys.stderr,
                )
                sys.exit(1)
            ...
            if pths:
                ...
                print(
                    "No .pth checkpoint found in Drive; downloading official Wav2Lip checkpoint from GitHub..."
                )
                ckpt = os.path.join(ckpt_dir, "wav2lip_gan.pth")
                ...
                if shutil.which("curl"):
                    ...
                    print(
                        "Error: neither curl nor wget is available; cannot download official Wav2Lip checkpoint.",
                        file=sys.stderr,
                    )
                    sys.exit(1)
                try:
                    ...
                    print(
                        "Error: failed to download official Wav2Lip checkpoint.",
                        file=sys.stderr,
                    )
                    sys.exit(1)
    ...
    if args.segments_file:
        ...
        if not os.path.isfile(args.segments_file):
            print(
                f"Error: segments file '{args.segments_file}' not found.",
                file=sys.stderr,
            )
            sys.exit(1)
        try:
    ...
    if args.tts_model is None and "xtts" in tts_model.lower():
        try:
            ...
            mgr = ModelManager(
                models_file=models_file, progress_bar=False, verbose=False
            )
            # 1) find language-specific non-XTTS model
            lang_models = [
                m
                for m in mgr.list_tts_models()
                if f"/{args.target_lang}/" in m and "xtts" not in m
            ]
            if lang_models:
                ...
                multi_models = [
                    m
                    for m in mgr.list_tts_models()
                    if m.startswith("tts_models/multilingual/") and "xtts" not in m
                ]
                new_model = multi_models[0] if multi_models else tts_model
            if new_model != tts_model:
                print(
                    f"Warning: XTTS-based model '{tts_model}' overridden to '{new_model}' for language blending",
                    file=sys.stderr,
                )
                tts_model = new_model
        except Exception:
    ...
    if args.accent_language and args.tts_model is None:
        try:
            ...
            if not multi_models:
                print(
                    f"Error: no multilingual TTS model available for accent hack",
                    file=sys.stderr,
                )
                sys.exit(1)
            ...
            if new_model != tts_model:
                print(
                    f"Info: accent-language hack enabled, switching TTS model from '{tts_model}' to '{new_model}'",
                    file=sys.stderr,
                )
                tts_model = new_model
        except Exception as e2:
            print(
                f"Warning: accent-language model selection failed: {e2}",
                file=sys.stderr,
            )
    sys.stderr.write("Instantiate TTS (It may take times)...\n")
    safe_globals = set()
    while True:
        try:
            tts = TTS(
            ...
            if m:
                ...
                if name in safe_globals:
                    print(
                        f"Error initializing TTS model '{tts_model}': {e}",
                        file=sys.stderr,
                    )
                    sys.exit(1)
                try:
                    ...
                    sys.stderr.write(
                        f"-- Instantiate TTS (retry #{len(safe_globals)}) "
                    )
                    sys.stderr.write(f"{name}\n")
                    continue
                except Exception as e2:
                    print(
                        f"Error adding safe global for '{name}': {e2}", file=sys.stderr
                    )
                    sys.exit(1)
    ...
    for i, seg in enumerate(segments):
        ...
        if not args.no_edit:
            ...
            print(
                f"Segment {i}: [{seg['start']:.2f}s -> {seg['end']:.2f}s]", flush=True
            )
            ...
            while True:
                choice = (
                    input("Press 'p'+Enter to play audio, or Enter to edit text: ")
                    .strip()
                    .lower()
                )
                if choice == "p":
        ...
        if args.preview:
            try:
                ...
                if not shutil.which("ffmpeg"):
                    print(
                        "Warning: ffmpeg not found; cannot generate preview",
                        file=sys.stderr,
                    )
                else:
                    ...
                    # try mplayer
                        ...
                        subprocess.run(["mplayer", "-quiet", preview_file])
                        played = True
    ...
    if args.lip_sync:
        ...
        try:
            ...
            print(
                f"Error: failed to concatenate lip-synced segments: {e}",
                file=sys.stderr,
            )
            sys.exit(1)
    else:
